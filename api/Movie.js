const express = require("express"); const puppeteer = require("puppeteer-core"); const axios = require("axios"); const cheerio = require("cheerio");

const router = express.Router(); const CHROMIUM_PATH = "/usr/bin/chromium";

// Existing movie scraper function remains unchanged const scrapeNkiri = async (query) => { console.log(🔍 Searching for: ${query});
...
const creator = "DRACULA"
    ...
    return { CREATOR: creator, STATUS: 200, title: movieTitle, description, download_link: finalDownloadLink };
} catch (error) {
...
// ✅ Improved function to scrape all episodes const scrapeEpisode = async (query) => { console.log(🔍 Searching for: ${query});
...
try {
    ...
    console.log(`🎉 Success! Final Download Link: ${finalDownloadLink}`);
    return { CREATOR: creator, STATUS: 200, title: foundEpisodeTitle, download_link: finalDownloadLink };
} catch (error) {
    console.error("❌ Error:", error.message);
    return { error: "Something went wrong", details: error.message };
}
...
// ✅ Existing endpoint for movies remains unchanged router.get("/", async (req, res) => { try { const { query } = req.query; if (!query) return res.status(400).json({ error: "Movie query is required" });
